...
-(void) requestOrderInfo
{
    ...
    [orderRequest nh_sendRequestWithCompletion:^(id response, BOOL success, NSString *message) {
        if (success) {
            ...
            weak_self.last_mileageLabek.text = [NSString stringWithFormat:@"%.1f",weak_self.rentalModel.last_mileage];
        }
    } error:^(NSError *error) {
...
-(void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI{
    ...
    //NSLog(@"%@",[NSString convertDataToHexStr:advertisementData[@"kCBAdvDataManufacturerData"]]);

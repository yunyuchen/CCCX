...
@interface YYNavScrollView()<UIScrollViewDelegate>

@end

@implementation YYNavScrollView
...
- (void)addImageViewsToScrollView {
    //图片视图；左边
    _imgVLeft = [[YYSiteView alloc] initWithFrame:CGRectMake(0.0, 0.0, kScreenWidth, 200)];
    _imgVLeft.frame = CGRectMake(10, 0, kScreenWidth - 20, 120);
    [self.scrollView addSubview:_imgVLeft];
    
    //图片视图；中间
    _imgVCenter = [[YYSiteView alloc] initWithFrame:CGRectMake(kScreenWidth, 0.0, kScreenWidth, 200)];
    _imgVCenter.frame = CGRectMake(kScreenWidth + 10 , 0, kScreenWidth - 20, 120);
    [self.scrollView addSubview:_imgVCenter];
    
    //图片视图；右边
    _imgVRight = [[YYSiteView alloc] initWithFrame:CGRectMake(kScreenWidth * 2.0, 0.0, kScreenWidth, 200)];
